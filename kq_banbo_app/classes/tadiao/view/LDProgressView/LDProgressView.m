...
- (void)drawLabelInRect:(CGRect)rect {
    ...
    label.font = [UIFont boldSystemFontOfSize:rect.size.height * pointsPerPixel/1.4];//默认去掉/1.7
    UIColor *baseLabelColor = [self.color isLighterColor] ? [UIColor blackColor] : [UIColor darkGrayColor];//whitecolor
    ...
    if (rect.size.width > width+8) {
        ...
        [label drawTextInRect:CGRectMake(rect.origin.x + size.width + 16, rect.origin.y, width, rect.size.height)];
    }
}
