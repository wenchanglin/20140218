...
// “‘œ¬∫Í∂®“Â”√”⁄HIK_MEDIAINFOΩ·ππ
#define FOURCC_HKMI			          0x484B4D49	// "HKMI" HIK_MEDIAINFOΩ·ππ±Íº«
// œµÕ≥∑‚◊∞∏Ò Ω	
#define SYSTEM_NULL			          0x0			// √ª”–œµÕ≥≤„£¨¥ø“Ù∆µ¡˜ªÚ ”∆µ¡˜	
#define SYSTEM_HIK                    0x1			// ∫£øµŒƒº˛≤„
...
//  ”∆µ±‡¬Î¿‡–Õ
#define VIDEO_NULL                    0x0           // √ª”– ”∆µ
#define VIDEO_H264                    0x1           // ±Í◊ºH.264∫Õ∫£øµH.264∂ºø…“‘”√’‚∏ˆ∂®“Â
#define VIDEO_MPEG4                   0x3           // ±Í◊ºMPEG4
#define VIDEO_MJPEG			          0x4
#define VIDEO_AVC264                  0x0100

// “Ù∆µ±‡¬Î¿‡–Õ
#define AUDIO_NULL                    0x0000        // √ª”–“Ù∆µ
#define AUDIO_ADPCM                   0x1000        // ADPCM 
#define AUDIO_MPEG                    0x2000        // MPEG œµ¡–“Ù∆µ£¨Ω‚¬Î∆˜ƒ‹◊‘  ”¶∏˜÷÷MPEG“Ù∆µ
// Gœµ¡–“Ù∆µ
#define AUDIO_RAW_DATA8		          0x7000        // ≤…—˘¬ Œ™8kµƒ‘≠ º ˝æ›
#define AUDIO_RAW_UDATA16	          0x7001        // ≤…—˘¬ Œ™16kµƒ‘≠ º ˝æ›£¨º¥L16
...
typedef struct _HIK_MEDIAINFO_				// modified by gb 080425
{
	...
	unsigned short  media_version;			// ∞Ê±æ∫≈£∫÷∏±æ–≈œ¢Ω·ππ∞Ê±æ∫≈£¨ƒø«∞Œ™0x0101,º¥1.01∞Ê±æ£¨01£∫÷˜∞Ê±æ∫≈£ª01£∫◊”∞Ê±æ∫≈°£
	unsigned short  device_id;				// …Ë±∏ID£¨±„”⁄∏˙◊Ÿ/∑÷Œˆ			
    
	unsigned short  system_format;          // œµÕ≥∑‚◊∞≤„
    unsigned short  video_format;           //  ”∆µ±‡¬Î¿‡–Õ
	
    unsigned short  audio_format;           // “Ù∆µ±‡¬Î¿‡–Õ
	unsigned char   audio_channels;         // Õ®µ¿ ˝  
    unsigned char   audio_bits_per_sample;  // —˘Œª¬ 
    unsigned int    audio_samplesrate;      // ≤…—˘¬  
    unsigned int    audio_bitrate;          // —πÀı“Ù∆µ¬Î¬ ,µ•Œª£∫bit
	
    unsigned int    reserved[4];            // ±£¡Ù
}HIK_MEDIAINFO;
#endif

typedef struct PLAYM4_SYSTEM_TIME //æ¯∂‘ ±º‰ 
{
	unsigned int dwYear;   //ƒÍ
	unsigned int dwMon;	   //‘¬
	unsigned int dwDay;	   //»’
	unsigned int dwHour;   // ±
	unsigned int dwMin;	   //∑÷
	unsigned int dwSec;	   //√Î
	unsigned int dwMs;	   //∫¡√Î
} PLAYM4_SYSTEM_TIME;

typedef enum _PLAYM4_PRIDATA_RENDER
{
    PLAYM4_RENDER_ANA_INTEL_DATA   = 0x00000001, //÷«ƒ‹∑÷Œˆ
    PLAYM4_RENDER_MD               = 0x00000002, //“∆∂Ø’Ï≤‚
    PLAYM4_RENDER_ADD_POS          = 0x00000004, //POS–≈œ¢∫Ûµ˛º”
}PLAYM4_PRIDATA_RENDER;

// ÷«ƒ‹–≈œ¢ø™πÿ
PLAYM4_API int __stdcall PlayM4_RenderPrivateData(int nPort, int nIntelType, int bTrue);
PLAYM4_API int PlayM4_RenderPrivateDataEx(int nPort, int nIntelType, int nSubType, int bTrue);
...
#define R_ANGLE_0   -1  //≤ª–˝◊™
#define R_ANGLE_L90  0  //œÚ◊Û–˝◊™90∂»
#define R_ANGLE_R90  1  //œÚ”“–˝◊™90∂»
...
{
	FEC_PLACE_WALL    = 0x1,		// ±⁄◊∞∑Ω Ω		(∑®œﬂÀÆ∆Ω)
	FEC_PLACE_FLOOR   = 0x2,		// µÿ√Ê∞≤◊∞		(∑®œﬂœÚ…œ)
	FEC_PLACE_CEILING = 0x3,		// ∂•◊∞∑Ω Ω		(∑®œﬂœÚœ¬)
...
typedef enum tagFECCorrectType
{
	...
	FEC_CORRECT_180 = 0x200,		// 180∂»Ω√’˝  £®∂‘”¶2P£©
	FEC_CORRECT_360 = 0x300,		// 360»´æ∞Ω√’˝ £®∂‘”¶1P£©

}FECCORRECTTYPE;

typedef struct tagCycleParam
{
	float	fRadiusLeft;	      // ‘≤µƒ◊Ó◊Û±ﬂX◊¯±Í
	float	fRadiusRight;	      // ‘≤µƒ◊Ó”“±ﬂX◊¯±Í
	float   fRadiusTop;		      // ‘≤µƒ◊Ó…œ±ﬂY◊¯±Í
	float   fRadiusBottom;	      // ‘≤µƒ◊Óœ¬±ﬂY◊¯±Í

}CYCLEPARAM;

typedef struct tagPTZParam
{
	float fPTZPositionX;		 // PTZ œ‘ æµƒ÷––ƒŒª÷√ X◊¯±Í
	float fPTZPositionY;		 // PTZ œ‘ æµƒ÷––ƒŒª÷√ Y◊¯±Í	

}PTZPARAM;


// ¥ÌŒÛ¬Î
#define FEC_ERR_ENABLEFAIL						0x500 // ”„—€ƒ£øÈº”‘ÿ ß∞‹
#define FEC_ERR_NOTENABLE						0x501 // ”„—€ƒ£øÈ√ª”–º”‘ÿ
#define FEC_ERR_NOSUBPORT						0x502 // ◊”∂Àø⁄√ª”–∑÷≈‰
#define FEC_ERR_PARAMNOTINIT					0x503 // √ª”–≥ı ºªØ∂‘”¶∂Àø⁄µƒ≤Œ ˝
#define FEC_ERR_SUBPORTOVER						0x504 // ◊”∂Àø⁄“—æ≠”√ÕÍ
#define FEC_ERR_EFFECTNOTSUPPORT				0x505 // ∏√∞≤◊∞∑Ω Ωœ¬’‚÷÷–ßπ˚≤ª÷ß≥÷
#define FEC_ERR_INVALIDWND						0x506 // ∑«∑®µƒ¥∞ø⁄
#define FEC_ERR_PTZOVERFLOW						0x507 // PTZŒª÷√‘ΩΩÁ
#define FEC_ERR_RADIUSINVALID					0x508 // ‘≤–ƒ≤Œ ˝∑«∑®
#define FEC_ERR_UPDATENOTSUPPORT				0x509 // ÷∏∂®µƒ∞≤◊∞∑Ω Ω∫ÕΩ√’˝–ßπ˚£¨∏√≤Œ ˝∏¸–¬≤ª÷ß≥÷
#define FEC_ERR_NOPLAYPORT						0x510 // ≤•∑≈ø‚∂Àø⁄√ª”–∆Ù”√
#define FEC_ERR_PARAMVALID						0x511 // ≤Œ ˝Œ™ø’
#define FEC_ERR_INVALIDPORT						0x512 // ∑«∑®◊”∂Àø⁄
#define FEC_ERR_PTZZOOMOVER						0x513 // PTZΩ√’˝∑∂Œß‘ΩΩÁ
#define FEC_ERR_OVERMAXPORT						0x514  // Ω√’˝Õ®µ¿±•∫Õ£¨◊Ó¥Û÷ß≥÷µƒΩ√’˝Õ®µ¿Œ™Àƒ∏ˆ
...
{
	unsigned int 	nUpDateType;			// ∏¸–¬µƒ¿‡–Õ
	unsigned int	nPlaceAndCorrect;		// ∞≤◊∞∑Ω Ω∫ÕΩ√’˝∑Ω Ω£¨÷ªƒ‹”√”⁄ªÒ»°£¨SetParamµƒ ±∫ÚŒﬁ–ß,∏√÷µ±Ì æ∞≤◊∞∑Ω Ω∫ÕΩ√’˝∑Ω Ωµƒ∫Õ
	PTZPARAM		stPTZParam;				// PTZ –£’˝µƒ≤Œ ˝
	CYCLEPARAM		stCycleParam;			// ”„—€ÕºœÒ‘≤–ƒ≤Œ ˝
	float			fZoom;					// PTZ œ‘ æµƒ∑∂Œß≤Œ ˝
	float			fWideScanOffset;		// 180ªÚ’ﬂ360∂»–£’˝µƒ∆´“∆Ω«∂»
	int				nResver[16];			// ±£¡Ù◊÷∂Œ
}FISHEYEPARAM;
...
// ”„—€≤•∑≈ø‚ƒ⁄≤ø∫Ø ˝£¨”√”⁄ µœ÷“ª–©ΩÁ√ÊPTZµƒ–ßπ˚
// µœ÷PTZ¥∞ø⁄…œµ„ª˜µƒ Û±ÍŒª÷√◊™ªªµΩ‘≠ ºÕºœÒ…œµƒµ„ 
