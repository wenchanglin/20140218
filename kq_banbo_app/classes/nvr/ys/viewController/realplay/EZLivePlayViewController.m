...
- (IBAction)ptzControlButtonTouchDown:(id)sender
{
    ...
    EZCameraInfo *cameraInfo = [_deviceInfo.cameraInfo dd_objectAtIndex:_cameraIndex];
...
//                       DDLogInfo(@"按下error is %@", error);
//                       DDLogInfo(@"anxiashebei:%@,%d,%d",cameraInfo.deviceSerial,cameraInfo.cameraNo,command);
                   }];
                 ...
                 cameraNo:cameraInfo.cameraNo
                  command:command
                   action:EZPTZActionStop
                    speed:0
                   result:^(NSError *error) {
      //                 NSLog(@"点击error is %@", error);
                   }];
    //DDLogInfo(@"dianjishebei:%@,%zd,%zd",cameraInfo.deviceSerial,cameraInfo.cameraNo,command);
}
